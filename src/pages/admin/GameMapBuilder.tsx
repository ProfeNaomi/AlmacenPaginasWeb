...
import { defaultGameLevels, GameLevel } from '../../lib/gameMap';
import { Map, Edit3, Settings, Plus, Layout, Type } from 'lucide-react';
...
export default function GameMapBuilder() {
  ...
  // Here an admin could edit a level configuration, e.g. mapping an App to the Level
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-2 text-lg">
            Configura el mapa de juego estilo Candy Crush. Asigna aplicaciones y simulaciones a cada nivel.
          </p>
        </div>
        <button className="flex items-center gap-2 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-400 hover:to-blue-400 text-white px-6 py-3 rounded-xl font-bold transition-all shadow-lg shadow-cyan-500/20">
            ...
            {selectedLevel ? (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                ...
                <div className="pt-6 border-t border-slate-800">
                  <button onClick={handleSave} className="w-full bg-cyan-500 hover:bg-cyan-400 text-slate-900 font-bold py-3 rounded-xl transition-colors">
                    Guardar Cambios
                  </button>
                </div>
