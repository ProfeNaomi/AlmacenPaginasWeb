...
import { Loader2, Plus, Save, Trash2, ArrowLeft, Video, Image as ImageIcon, AlignLeft, Layout, Settings, FileQuestion } from 'lucide-react';
...
export default function ContentMaker() {
  ...
  const addElement = (type: LessonElement['type']) => {
    const newPages = [...pages];
    newPages[activePageIndex].elements.push({
  ...
  const updateElement = (elementId: string, content: string) => {
    const newPages = [...pages];
    const element = newPages[activePageIndex].elements.find(e => e.id === elementId);
    if (element) {
  ...
  const removeElement = (elementId: string) => {
    const newPages = [...pages];
    newPages[activePageIndex].elements = newPages[activePageIndex].elements.filter(e => e.id !== elementId);
    setPages(newPages);
  };
          ...
          {activeTab === 'pages' && pages[activePageIndex] && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {pages[activePageIndex].elements.map((el, i) => (
                  <div key={el.id} className="relative group bg-slate-950 border border-slate-800 rounded-xl p-4">
                    <button 
                      onClick={() => removeElement(el.id)}
                      className="absolute -top-3 -right-3 bg-red-500 text-white p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600"
                    >
                      <X className="w-4 h-4" />
                    </button>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 pt-4">
                  <button onClick={() => addElement('text')} className="flex flex-col items-center justify-center p-3 border border-slate-700 border-dashed rounded-xl text-slate-400 hover:text-cyan-400 hover:border-cyan-500 hover:bg-cyan-950/20 transition-all text-xs font-bold gap-2">
                    <AlignLeft className="w-5 h-5" /> Texto
                  </button>
                  <button onClick={() => addElement('video')} className="flex flex-col items-center justify-center p-3 border border-slate-700 border-dashed rounded-xl text-slate-400 hover:text-red-400 hover:border-red-500 hover:bg-red-950/20 transition-all text-xs font-bold gap-2">
                    <Video className="w-5 h-5" /> Video
                  </button>
                  <button onClick={() => addElement('image')} className="flex flex-col items-center justify-center p-3 border border-slate-700 border-dashed rounded-xl text-slate-400 hover:text-blue-400 hover:border-blue-500 hover:bg-blue-950/20 transition-all text-xs font-bold gap-2">
                    <ImageIcon className="w-5 h-5" /> Imagen
                  </button>
                  <button onClick={() => addElement('app')} className="flex flex-col items-center justify-center p-3 border border-slate-700 border-dashed rounded-xl text-slate-400 hover:text-emerald-400 hover:border-emerald-500 hover:bg-emerald-950/20 transition-all text-xs font-bold gap-2">
                    <Layout className="w-5 h-5" /> App / Iframe
                  </button>
                  <button onClick={() => addElement('gadget')} className="flex flex-col items-center justify-center p-3 border border-slate-700 border-dashed rounded-xl text-slate-400 hover:text-amber-400 hover:border-amber-500 hover:bg-amber-950/20 transition-all text-xs font-bold gap-2">
